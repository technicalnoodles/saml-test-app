...
We need to import another library called `reduxThunk`. This library acts as a _middleware_ and allows asynchronous Redux actions to take place. The second thing we need to import is all of our _reducers_. The `import reducers from './reducers'` works without specifying a file because Redux looks for a file called `index.js` in the `reducers` directory that should contain all of the reducers.
...
Next we need to edit our `store`. Initally there was a function that returned an empty object. This became the intial state for the entire `store`. Now we are passing `reducers` to it, which is going to set the store default state to what is defined in the reducers.
...
Now that our logic is setup, we need to edit the header to use the new logic. In the `Header` function of our component, we will remove the the `login` a tag and replace it with the `renderLogin` function. We need to wrap it in curly braces though so it does not get treated as HTML.
