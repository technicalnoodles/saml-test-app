...
After being able to successfully create a user or find a user, we need to be able to set a cookie for them, so their session is persistent. To do this, we need be able to serialize the user, deserialize the user, and enable cookies.
...
It should ask for a login and permissions. After accepting them the server will crash. This is because as of May 2022, an update to Passport has broken compatibility with `cookie-session` because it does not have the `regenerate` function or the `save` function. The owner of Passport recommends using an older version till a fix is out. To do this we need to uninstall passport and install the previous version The below commands will do just that:
...
After starting the server back up and running through the authentication again, we should see `Cannot GET /auth/cisco/callback`. This is expected but the authentication flow did finish successfully.
...
Finally, we need to an API to check if there is a user logged in. This is what our frontend will use to check if it can say someone is logged in or not.
