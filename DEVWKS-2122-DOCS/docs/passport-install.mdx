...
The first thing we need to do to make sure everything will work properly is to add our WebEx integration ID and Secret a files called `keys.js`. This file is does not exist yet, but there is an example file called `exKey.js` located in the `config` directory.
...
Now we need to rename the file to `keys.js`. This will allow us to make sure all of the next steps will work.
...
The last step to amke sure our authentication flow works, is to import and call everything we need in the `index.js` file.
...
We have a working authentication flow and have finished the intial setup of Passport. We can now move on to adding the database.
