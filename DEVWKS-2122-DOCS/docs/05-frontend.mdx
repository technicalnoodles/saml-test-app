...
Please paste this code into the `authReducer.js` file:
...
In the `authReducer` file, we import the _type_ of _action_ we want to be able to use.

Next we define the function that handles these actions.
...
In the `index.js` file paste the below code:
...
Delete everything in the `index.js` file and paste the below.
...
We need to import another library called `reduxThunk`. This library is a _middleware_ and allows asynchronous Redux actions to take place. The second thing we need to import are the _reducers_. The `import reducers from './reducers'` works without specifying a file because Redux looks for a file called `index.js` in the `reducers` directory that should contain all the reducers.
...
Delete everything in the `App.js` file and paste in the below code:
...
We will come back to the `App.js` file in a little bit so we can refactor the returned code to use the Router. But first we will edit the `Header.js` component to change the login and logout button after checking the user.
...
Delete everything in the `Header.js` file in the `scr/components` directory and paste in the below code:
...
We first import `useSelector` from Redux. This function allows us to access the state of the Redux store from anywhere without needing to do something called prop-drilling. Prop-drilling introduces a multitude of dependency issues in an application and can cause significant issues as a project gets larger.

We import `Link` second from the React Router. This will tell React which page to show when it is selected.
...
Back in the `App.js`, we need to refactor the code to use the React Router and specify the routes for specific components.

Remove the code in the `App.js` file and paste in the below:
...
Now we have a fully functioning full-stack application with WebEx OAuth and permission checking.
